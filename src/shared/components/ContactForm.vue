...
const parsedSocials = computed(() => {
  return portfolioData.socials.map(social => ({
    name: social.name,
    url: social.url,
    svgPath: svgPaths[social.name],
    colorClass: hoverClasses[social.name] || 'text-neutral-500 hover:text-white transition-colors'
  })).filter(s => s.svgPath);
});
</script>
