<template>
  <div class="w-full max-w-lg bg-neutral-900/60 backdrop-blur-md p-8 border-l-4 border-[#00ffff] rounded-none shadow-[0_0_15px_rgba(0,255,255,0.3)] pointer-events-auto">
    ...
    <div class="mt-8 pt-6 border-t border-neutral-800 flex justify-center space-x-6">
      <a v-for="social in parsedSocials" :key="social.name" :href="social.url" target="_blank" rel="noopener noreferrer" :class="social.colorClass" :title="social.name">
        <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path :d="social.svgPath"/></svg>
      </a>
    </div>
  </div>
</template>
...
const hoverClasses = {
  'GitHub': 'text-neutral-500 hover:text-white transition-colors',
  'LinkedIn': 'text-neutral-500 hover:text-[#00ffff] transition-colors',
  'Instagram': 'text-neutral-500 hover:text-[#ff00ff] transition-colors',
  'Facebook': 'text-neutral-500 hover:text-[#3b5998] transition-colors'
};

const parsedSocials = computed(() => {
  const socials = [];
  // Match lines like [![GitHub](...)](url)
  const regex = /\[!\[(.*?)\]\([^)]+\)\]\(([^)]+)\)/g;
  let match;
  while ((match = regex.exec(rawReadme)) !== null) {
    const name = match[1];
    const url = match[2];
    if (svgPaths[name]) {
      socials.push({
        name,
        url,
        svgPath: svgPaths[name],
        colorClass: hoverClasses[name] || 'text-neutral-500 hover:text-white transition-colors'
      });
    }
  }
  return socials;
